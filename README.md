...
M0–M5 全部完成，且都用真实模型（不只是 mock）跑通端到端验证；下一阶段 **M6（能力扩展）** 已完成设计、尚未开工：
...
三种运行模式：`standard`（默认六阶段）、`quick`（跳过 critique/revise/vote）、`planning`（按主题拆分，支持综合技术规划这类长输出）。代码结构：`apps/cli`（命令行）、`apps/api`（Fastify + Postgres，Conversation/Run API + SSE 事件流）、`apps/web`（Next.js 前端），核心编排逻辑抽在 `packages/orchestrator`，CLI 与 API 共用同一份实现。BYOK 让用户从受限 provider 白名单里选厂商、自带 API key 跑协商（匿名 workspace、cookie 识别、可选加密保存），每个 run 有成本上限（默认 $5）在超阈值时于下一阶段前熔断。
