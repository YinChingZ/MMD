...
**M0（协议加固）+ M1（CLI 原型）+ M1.5（收敛验证，Go 决策）+ v0.2（Planning Mode 长输出支持）+ M2（Backend API）+ M3（Web MVP）+ M4 第一阶段（BYOK 平台）均已完成**，且都用真实模型（不只是 mock）跑通过端到端验证。`apps/cli` 支持三种模式：`standard`（默认六阶段）、`quick`（跳过 critique/revise/vote）、`planning`（按主题拆分、支持综合技术规划这类长输出）。`apps/api` 把同一套 orchestrator 逻辑（现已提取为 `packages/orchestrator`，CLI 和 API 共用）搬到了 Fastify + Postgres 服务端：Conversation/Run API、SSE 事件流（断线重连可按 `Last-Event-ID` 回放）、run 结果持久化。`apps/web` 是消费这套 API 的 Next.js 前端：提问、选模型、实时看运行进度、展开查看共识背后的原始 claims、复制最终答案。M4 第一阶段是 BYOK（自带 API key）——不做账号登录体系，用户从一个受限的 provider 白名单里选厂商、填自己的 key 来跑协商，默认不落库、可选择加密保存在匿名 workspace 下（cookie 识别，无需注册）。下一步是 M4 剩余部分：成本估算/熔断、分享链接。详见 [multi-model-deliberation-dev-roadmap.md](multi-model-deliberation-dev-roadmap.md)（含真实测试的发现和数据）。
...
| `GET /api/providers` | 列出 BYOK 白名单 provider（`providerId`/`displayName`，不下发 baseUrl） |
...
| `POST /api/conversations/:id/runs` | 发起一次协商 run（`question`/`mode`/可选 `modelIds`（服务端注册表）/可选 `byokModels`（自带 provider+key，或引用已保存的 `savedKeyId`）） |
...
模型列表下方是"Add your own API key"（M4 BYOK）：从白名单选 provider（OpenAI/DeepSeek/OpenRouter/Volcengine）、填自己的 API key 和模型 id，加入本次 run；可选"记住这个 key"把它加密保存在这台设备的匿名 workspace 下，下次访问在"Saved keys on this device"里一键复用，浏览器不需要再次持有明文。
