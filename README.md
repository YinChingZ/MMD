...
**M0（协议加固）+ M1（CLI 原型）+ M1.5（收敛验证，Go 决策）+ v0.2（Planning Mode 长输出支持）+ M2（Backend API）+ M3（Web MVP）+ M4（BYOK 平台）+ M5.1（成本熔断）+ M5.2（CI）+ M5.3（限流与数据清理）均已完成**，且都用真实模型（不只是 mock）跑通过端到端验证。`apps/cli` 支持三种模式：`standard`（默认六阶段）、`quick`（跳过 critique/revise/vote）、`planning`（按主题拆分、支持综合技术规划这类长输出）。`apps/api` 把同一套 orchestrator 逻辑（现已提取为 `packages/orchestrator`，CLI 和 API 共用）搬到了 Fastify + Postgres 服务端：Conversation/Run API、SSE 事件流（断线重连可按 `Last-Event-ID` 回放）、run 结果持久化。`apps/web` 是消费这套 API 的 Next.js 前端：提问、选模型、实时看运行进度、展开查看共识背后的原始 claims、复制最终答案。M4 是 BYOK（自带 API key）——不做账号登录体系，用户从一个受限的 provider 白名单里选厂商、填自己的 key 来跑协商，默认不落库、可选择加密保存在匿名 workspace 下（cookie 识别，无需注册）。M5.1 是成本熔断——每次 run 都有一个 USD 成本上限（默认 $5，可在发起 run 时覆盖），跨阶段累加实际花费，超过阈值就在下一阶段开始前停止 run，避免 BYOK 用户在不知情的情况下烧穿自己的账号额度。M5.2 是 CI——`.github/workflows/ci.yml` 用真实 Postgres service container 跑全 workspace 的 build/migrate/test；实现过程中发现并修复了一个此前从未暴露过的真实 bug：根 `build` 脚本按 npm workspaces 默认字母序执行，与 TypeScript project references 要求的依赖顺序不一致，全新 checkout（比如 CI）上必现构建失败，本地开发因为 `dist/` 目录从未被清空过而从未撞见。M5.3 是限流与数据清理——`POST /api/conversations/:id/runs` 按 workspace 限流（10 次/分钟），新增 `DELETE /api/conversations/:id`（级联删除该会话下所有数据），以及一个独立的清理脚本按 `last_seen_at` 删除超过 30 天不活跃的匿名 workspace；同时给之前完全没有 `ON DELETE CASCADE` 的外键补了一次迁移，否则删除接口会直接因为外键约束报错。下一步是 M5.4：部署文档/Dockerfile。详见 [multi-model-deliberation-dev-roadmap.md](multi-model-deliberation-dev-roadmap.md)（含真实测试的发现和数据）。
