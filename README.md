...
**M0（协议加固）+ M1（CLI 原型）+ M1.5（收敛验证，Go 决策）+ v0.2（Planning Mode 长输出支持）+ M2（Backend API）+ M3（Web MVP）+ M4（BYOK 平台）+ M5.1（成本熔断）均已完成**，且都用真实模型（不只是 mock）跑通过端到端验证。`apps/cli` 支持三种模式：`standard`（默认六阶段）、`quick`（跳过 critique/revise/vote）、`planning`（按主题拆分、支持综合技术规划这类长输出）。`apps/api` 把同一套 orchestrator 逻辑（现已提取为 `packages/orchestrator`，CLI 和 API 共用）搬到了 Fastify + Postgres 服务端：Conversation/Run API、SSE 事件流（断线重连可按 `Last-Event-ID` 回放）、run 结果持久化。`apps/web` 是消费这套 API 的 Next.js 前端：提问、选模型、实时看运行进度、展开查看共识背后的原始 claims、复制最终答案。M4 是 BYOK（自带 API key）——不做账号登录体系，用户从一个受限的 provider 白名单里选厂商、填自己的 key 来跑协商，默认不落库、可选择加密保存在匿名 workspace 下（cookie 识别，无需注册）。M5.1 是成本熔断——每次 run 都有一个 USD 成本上限（默认 $5，可在发起 run 时覆盖），跨阶段累加实际花费，超过阈值就在下一阶段开始前停止 run，避免 BYOK 用户在不知情的情况下烧穿自己的账号额度。下一步是 M5.2：CI。详见 [multi-model-deliberation-dev-roadmap.md](multi-model-deliberation-dev-roadmap.md)（含真实测试的发现和数据）。
