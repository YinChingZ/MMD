...
**M0（协议加固）+ M1（CLI 原型）+ M1.5（收敛验证，Go 决策）+ v0.2（Planning Mode 长输出支持）+ M2（Backend API）+ M3（Web MVP）+ M4（BYOK 平台）+ M5.1（成本熔断）+ M5.2（CI）+ M5.3（限流与数据清理）+ M5.4（部署文档/Dockerfile）+ M5.5（分享链接）均已完成，M5 到此全部完成**，且都用真实模型（不只是 mock）跑通过端到端验证。`apps/cli` 支持三种模式：`standard`（默认六阶段）、`quick`（跳过 critique/revise/vote）、`planning`（按主题拆分、支持综合技术规划这类长输出）。`apps/api` 把同一套 orchestrator 逻辑（现已提取为 `packages/orchestrator`，CLI 和 API 共用）搬到了 Fastify + Postgres 服务端：Conversation/Run API、SSE 事件流（断线重连可按 `Last-Event-ID` 回放）、run 结果持久化。`apps/web` 是消费这套 API 的 Next.js 前端：提问、选模型、实时看运行进度、展开查看共识背后的原始 claims、复制最终答案。M4 是 BYOK（自带 API key）——不做账号登录体系，用户从一个受限的 provider 白名单里选厂商、填自己的 key 来跑协商，默认不落库、可选择加密保存在匿名 workspace 下（cookie 识别，无需注册）。M5.1 是成本熔断——每次 run 都有一个 USD 成本上限（默认 $5，可在发起 run 时覆盖），跨阶段累加实际花费，超过阈值就在下一阶段开始前停止 run，避免 BYOK 用户在不知情的情况下烧穿自己的账号额度。M5.2 是 CI——`.github/workflows/ci.yml` 用真实 Postgres service container 跑全 workspace 的 build/migrate/test；实现过程中发现并修复了一个此前从未暴露过的真实 bug：根 `build` 脚本按 npm workspaces 默认字母序执行，与 TypeScript project references 要求的依赖顺序不一致，全新 checkout（比如 CI）上必现构建失败，本地开发因为 `dist/` 目录从未被清空过而从未撞见。M5.3 是限流与数据清理——`POST /api/conversations/:id/runs` 按 workspace 限流（10 次/分钟），新增 `DELETE /api/conversations/:id`（级联删除该会话下所有数据），以及一个独立的清理脚本按 `last_seen_at` 删除超过 30 天不活跃的匿名 workspace；同时给之前完全没有 `ON DELETE CASCADE` 的外键补了一次迁移，否则删除接口会直接因为外键约束报错。M5.4 是部署文档/Dockerfile——`apps/api/Dockerfile`、`apps/web/Dockerfile`、`docker-compose.yml`、[docs/deployment.md](docs/deployment.md)；已用真实 Docker 完整验证过 `docker build` + `docker compose up --build` 起两个容器、走完"创建会话 → 添加 BYOK key → 提交 run → 看到结果"的完整流程，且发现并修了两个真实问题——apps/api 的各 workspace 包 `package.json` 的 `"main"` 直接指向 `.ts` 源文件，编译后用纯 `node dist/main.js` 跑会直接报模块找不到，改成 runtime 阶段用 `tsx`（跟 `npm run dev` 同一条路径）；以及 apps/web 的 `API_BASE_URL` 只在构建期生效一次（写进 Next.js 的 rewrite 清单），运行期改环境变量没有用。M5.5 是分享链接——完成页可以生成一个公开只读的 `/share/[token]` 链接（无需 workspace cookie 即可访问，可随时撤销），实现过程中把根 layout 的侧边栏 chrome 挪进了一个新的 route group，确保分享页面对匿名访客干干净净、不触发 workspace cookie 签发、也不出现任何"新建会话"之类的操作入口。详见 [multi-model-deliberation-dev-roadmap.md](multi-model-deliberation-dev-roadmap.md)（含真实测试的发现和数据）。
...
| `POST /api/conversations/:id/runs` | 发起一次协商 run（`question`/`mode`/可选 `modelIds`（服务端注册表）/可选 `byokModels`（自带 provider+key，或引用已保存的 `savedKeyId`）/可选 `costLimitUsd`，不传时默认 $5——见下方"成本熔断"） |
