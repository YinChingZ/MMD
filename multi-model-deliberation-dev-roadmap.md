...
| M4 第一阶段：BYOK 平台 | ✅ 完成 | 见下方"M4 第一阶段补充：BYOK — 实际结果"；不做账号体系，用户自带 API key |
| M4 后续：成本估算/熔断、分享链接 | 未开始 | 下一步 |
...
### M4 后续：成本估算/熔断、分享链接（未开始）

- 成本估算展示、单次 run 的成本上限熔断（超预算提前终止并提示，而不是等跑完才发现超支）、分享链接——相对独立、改动更小，是 BYOK 之后的下一批工作，这次没有深入设计。
...
M0、M1、M1.5、v0.2、M2、M3、M4 第一阶段（BYOK 平台）均已完成并通过真实验证（BYOK 见上方"M4 第一阶段补充"一节的实测数据）。下一步是 M4 剩余部分：成本估算展示、单次 run 的成本上限熔断、分享链接。

其他已知的、不阻塞下一步但值得记录的后续待办：
- BYOK 完全自定义 baseUrl（需要内网地址过滤、DNS rebinding 防护、重定向校验、IP 编码解析等 SSRF 加固）、Anthropic/Google 专用 adapter、加密密钥轮换机制、删除已保存 key 的接口——见上方"M4 第一阶段补充"一节。
