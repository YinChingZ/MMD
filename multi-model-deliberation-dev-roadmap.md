...
配套文档：[multi-model-deliberation-tech-design.md](/Users/xyz91928/Documents/Codex/2026-07-03/yo/outputs/multi-model-deliberation-tech-design.md)、[docs/protocol.md](docs/protocol.md)、[docs/litellm-integration.md](docs/litellm-integration.md)
...
| M2' LiteLLM-first 集成转向 | 进行中 | 已落地 Python/Pydantic PoC、quick/standard mode、LiteLLM custom provider、Proxy HTTP smoke 和 versioned `return_trace` contract；下一步是真实模型 smoke、planning mode、Router/callback、upstream readiness |
...
3. **M2'.2 Python orchestrator**：✅ 已完成 quick/standard。支持 fan-out quorum、partial、explicit vote、major/critical objection 分类和 repair retry；planning mode 尚未迁移。
4. **M2'.3 LiteLLM provider integration**：进行中。已支持 `analysis_models`、`coordinator_model`、`mmd_mode`、`quorum_ratio`、`return_trace`，并通过 LiteLLM Proxy HTTP smoke 验证 `mmd.trace_version === 1`；下一步是真实模型配置 smoke、Router/callback 接入和 usage 聚合。
5. **M2'.4 planning mode + advanced config**：未开始。迁移 planning mode、max topics、递归保护、预算/熔断。
...
1. **真实模型 Proxy smoke**：用真实 `analysis_models` / `coordinator_model` 跑通 `model: mmd-fusion`，验证 auth、timeout、repair retry、quorum partial 和 `mmd` trace 契约。
2. **Python planning mode**：迁移 outline、per-topic standard 并行、section compose、deterministic executive summary，并保持 TypeScript CLI 行为一致。
3. **LiteLLM Router/callback 集成**：内部调用优先走 Router，补 usage 聚合、callback/logging trace 落点和 provider 异常映射。
4. **Upstream readiness**：整理目录、命名、错误类型、配置字段和测试形态，决定提交 LiteLLM upstream PR 还是先发布 external custom provider package。
...
- LiteLLM 集成版已明确第一版 trace 返回位置：默认 response 保持 OpenAI-compatible；`return_trace=true` 时顶层 `mmd` metadata 返回 claim/review/vote/classification/quorum/failure trace。后续还需要把同一 trace 写入 callback/logging，便于生产环境审计。
