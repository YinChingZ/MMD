...
| M2' LiteLLM-first 集成转向 | 进行中 | 本 branch 的新主线：暂停自建 Backend API，以 LiteLLM 适配便利、upstream 接受度和开源影响力为最高目标 |
...
1. **M2'.0 LiteLLM-native compatibility spike**：验证 LiteLLM custom provider / native provider 能否包装 `mmd/fusion` 虚拟模型，先用 mock completion 返回 OpenAI-compatible response，并从第一版就按 upstream PR 形态组织。
2. **M2'.1 Python protocol port**：把 zod schema、quorum、`classifyCandidate`、structured repair 移植成 Python/Pydantic，并与现有 TypeScript 测试行为对齐。
3. **M2'.2 Python orchestrator**：用 `litellm.acompletion` / LiteLLM Router 跑通 `quick` 和 `standard`，保留 fan-out quorum、partial 标记和 repair retry。
4. **M2'.3 LiteLLM provider integration**：支持 `analysis_models`、`coordinator_model`、`mmd_mode`、`quorum_ratio`、`return_trace` 等配置，通过 LiteLLM Proxy 暴露 `mmd-fusion`。
5. **M2'.4 planning mode + advanced config**：迁移 planning mode、max topics、递归保护、预算/熔断。
6. **M2'.5 upstream readiness**：整理成符合 LiteLLM 风格的 PR 或外部 custom provider package。
...
M0、M1、M1.5 均已完成并给出 Go 决策，v0.2 Planning Mode 作为额外能力也已上线并通过真实测试。下一步不再是原 M2 Backend API，而是 M2' LiteLLM-first integration：先做 upstream-shaped provider 兼容性验证，再把协议核心迁移到 Python/Pydantic，并通过 LiteLLM Proxy 暴露一个 `mmd/fusion` 风格的虚拟模型。
...
- LiteLLM 集成版需要明确 trace metadata 的返回位置：默认 response 必须保持 OpenAI-compatible，完整 claim/review/vote/classification trace 应默认关闭，或进入 callback/logging。
