...
目前的验证基线：Python 177 tests（`litellm[proxy]` 安装时；含 8 个真实 Proxy 子进程 e2e test；未安装时为 164 tests + 13 个自动 skip）、TypeScript 102 tests，各自构建通过；scripted Proxy smoke 和核心真实模型 smoke 已运行。真实 smoke 未保存质量、成本或 latency artifact，不能作为性能或效果基准。
