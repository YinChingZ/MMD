...
| M6：能力扩展（流式/工具/多模态/自定义 JSON 输出） | 🚧 进行中（M6.1–M6.4 已完成） | 设计已完成，见 [streaming-tools-multimodal-json.md](streaming-tools-multimodal-json.md)；按该文档的推荐顺序拆成 6 个子项（M6.1–M6.6）。详见下方"M6"一节 |
...
## M6：能力扩展（流式输出 / 工具调用 / 多模态输入 / 自定义 JSON 输出）— M6.1–M6.4 已完成，M6.5–M6.6 规划中
...
M0–M5 全部完成并通过真实验证（M5 五个子项见上方各"实际结果"一节）。**M6（能力扩展）中 M6.1–M6.4 均已完成**（见上方各自"已完成"一节）：M6.1 用户自定义 JSON 输出、M6.2 per-model 细粒度进度事件、M6.3 claim/item 级渐进式解析、M6.4 compose 阶段 token 级流式（M6.3/M6.4 共享同一个 provider `completeStream` 接口，按设计文档建议一次设计、分两步落地）。M6.4 开工前确认的产品范围问题：planning 模式的 section-compose 也要做 token 流式（两种模式对称实现，成本差异只在前端渲染多路并发打字机）。M6.3 实现时发现并修复了一个设计阶段没预料到的真实 bug——`apps/api` 的 `RoutingProvider` 完全没有转发 `completeStream`，导致哪怕接了真实的流式 provider，流式在生产路径里也永远不会触发；已修复为总是转发、按路由 provider 是否支持流式决定转发还是退化为普通 `complete()`。**下一步建议做 M6.5（多模态输入）或 M6.6（tool calling）**——两者影响面都比 M6.1–M6.4 更大（M6.5 涉及 provider/API/Web/存储选型，M6.6 涉及 budget 调整和"同质化/成本"取舍），开工前请先逐条确认设计文档第 6 节里跟它们各自相关的待决问题（M6.5：图片是否长期保留影响存储选型；M6.6：propose/critique 的工具开关合一还是拆开、`web_search` 结果是否跨模型共享缓存）。
