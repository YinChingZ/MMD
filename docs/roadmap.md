...
| M6：能力扩展（流式/工具/多模态/自定义 JSON 输出） | 🚧 规划中 | 设计已完成，见 [streaming-tools-multimodal-json.md](streaming-tools-multimodal-json.md)；按该文档的推荐顺序拆成 6 个子项（M6.1–M6.6），尚未开工。详见下方"M6"一节 |
...
## M6：能力扩展（流式输出 / 工具调用 / 多模态输入 / 自定义 JSON 输出）— M6.1 已完成，M6.2–M6.6 规划中
...
- `FanoutOptions` 加可选 `onSettled` 回调，`fanOutWithQuorum` 每个模型 settle 即回调（不再等 `Promise.all` 整体完成）；orchestrator 六处调用点各加一行，emit 新 `RunEventType` `model_responded {modelId, ok, latencyMs}`。
...
M0–M5 全部完成并通过真实验证（M5 五个子项见上方各"实际结果"一节）。**M6（能力扩展）中 M6.1（用户自定义 JSON 输出）已完成**（见上方 M6.1"已完成"一节），设计文档第 6 节里跟它相关的两个待决问题已拍板（只支持 JSON Schema、`userOutput` 附加在 `GET /result` 里）。**下一步建议做 M6.2（per-model 细粒度进度事件）**——改动最小（`fanout.ts` 加一个可选回调 + orchestrator 六处调用点各加一行）、投入产出比最高，且不需要额外的产品/协议拍板。M6.3–M6.6 仍未开工，开工前请先逐条确认设计文档第 6 节里跟它们各自相关的待决问题。
