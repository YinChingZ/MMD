...
## M6：能力扩展（流式输出 / 工具调用 / 多模态输入 / 自定义 JSON 输出）— 规划中
...
M0–M5 全部完成并通过真实验证（M5 五个子项见上方各"实际结果"一节）。**下一阶段是 M6（能力扩展）**：流式输出 / 工具调用 / 多模态输入 / 自定义 JSON 输出，设计已完成（[streaming-tools-multimodal-json.md](streaming-tools-multimodal-json.md)），按推荐顺序拆成 M6.1–M6.6（见上方"M6"一节），尚未开工。**建议第一步做 M6.1（用户自定义 JSON 输出）**——它牵扯的产品语义最需要先钉死（第 6 节的待决问题多数挂在它上面）；如果只想先拿一个立竿见影的 UX 改善，M6.2（per-model 进度事件）改动最小、投入产出比最高，可以先插进去。开工前先逐条确认设计文档第 6 节需要产品/协议拍板的问题。
