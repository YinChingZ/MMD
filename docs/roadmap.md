...
| M5：项目收尾与生产就绪 | ✅ 全部完成 | 见下方"M5"一节；成本熔断→CI→限流/清理→部署→分享链接，五个子项均已完成并做过真实验证 |
| M6：能力扩展（流式/工具/多模态/自定义 JSON 输出） | ✅ 完成（M6.1–M6.6） | 自定义 JSON、细粒度进度、流式、图片输入和 OpenAI BYOK web search 均已完成。详见下方"M6"一节 |
...
- **换成真正跨厂商组合后的复测（2026-07-04）**：把三个模型换成 OpenAI GPT-5.5 / DeepSeek v4 Pro / Google Gemini 3.1 Pro（经 OpenRouter 统一接入），重跑了梅西 vs C罗和 996 这两个问题：
  ...
  - 详见 [protocol.md](protocol.md)"已观察到的一个分类边界情况"一节：这次复测还发现一个分类边界情况——全票 critical 反对会被归类为 `disputed` 而不是更符合直觉的 `rejected`，是规则按预期生效，不是 bug，但标注为可能的后续优化点。
...
- 新增 **Outline 阶段**：单一 coordinator 调用把问题拆成最多 8 个主题，然后现有六阶段协议对每个主题**并行**独立跑一遍（详细设计和"为什么 outline 阶段可以用单一 coordinator 而不违反 4.1 原则"的推理见 [protocol.md](protocol.md) "v0.2 Planning Mode" 一节）。
...
- **两处刻意偏离原文档**：(1) 模型选择改为服务端 `models.config.json` 注册表，创建 run 的请求只能从中选 `modelIds` 子集，不接受客户端自带 `provider`/`baseUrl`（避免 SSRF 和客户端自带 API key）；(2) 不引入 Redis——单进程内存 SSE 广播器 + Postgres 持久化（供断线重连回放）已满足 M2 验收标准，跨进程事件分发留到真正需要水平扩展时再做。
...
### M5.4 部署文档/Dockerfile — 实际结果（已完成，2026-07-05）

原计划里"runtime 阶段只拷贝 `dist`/`node_modules`/`package.json`，跑 `node dist/main.js`"这条具体做法**没有照搬**——实现前先不依赖 Docker 做了一次本地模拟（手动按 Dockerfile 两阶段的拷贝范围复制文件到临时目录，再直接跑 `node`/`npm start`），结果直接复现了一个真实报错：`packages/*` 每个包的 `package.json` 的 `"main"`/`"types"` 都指向各自的 `src/index.ts`（M5.1 时期的既有设计），纯 Node 跑编译后的 `apps/api/dist/main.js` 会在 `require("@mmd/protocol")` 时顺着符号链接找到未编译的 `.ts` 源文件，直接 `ERR_MODULE_NOT_FOUND`。改为 runtime 阶段用 `npm run start`（即 `tsx src/main.ts`，跟 `npm run dev` 已经在用的方式完全一致）而不是 `node dist/main.js`，`tsc` 构建保留但只作为构建期类型检查关卡，runtime 不使用其产物。同样的本地模拟方式验证过：只拷贝 `packages/*/{package.json,src}` + `apps/api/{package.json,src}` + 根 `node_modules`（不含任何 `dist`），`npm run start` 能正常监听端口，且 `GET /health`、`POST /api/conversations`、`GET /api/conversations` 均返回正确结果。

`apps/web` 一侧原计划的"加 `output: "standalone"`"照原样保留，且实测确认了一个隐含假设成立：这个 app 对 `@mmd/protocol`/`@mmd/orchestrator` 的引用全部是 `import type`（M4 阶段"Turbopack 无法打包 workspace 包运行时值"这一发现的直接结果），所以 standalone 产物完全不含 `@mmd/*`（用 `find`/`grep` 直接检查过 `.next/standalone` 目录确认），不需要在 Dockerfile 里为 apps/web 做类似 apps/api 那种特殊处理。构建过程中额外发现并修了一个问题：这个 worktree 嵌套在主仓库目录树内部，主仓库根目录恰好也有一份 `package-lock.json`，导致 Next.js 把它误判成 monorepo root（"multiple lockfiles" 警告），standalone 产物的目录结构因此不对（嵌套进了 `.claude/worktrees/...` 路径）——加了 `next.config.ts` 的 `turbopack.root` 显式指向仓库根目录后解决，这个具体触发条件是 worktree 特有的巧合，但显式设置本身对任何环境都更正确、不依赖"猜测最近的 lockfile"这种隐式行为。

**意外发现且已实测验证的一个非直觉细节**：`apps/web` 的 `API_BASE_URL` 不是运行期变量——Next.js 的 `rewrites()` 只在 `next build` 时求值一次，结果写进 `.next/routes-manifest.json` 的 rewrite 目标，请求到来时不会重新计算。也就是说这个变量必须作为 Docker build arg（构建期），`docker run -e API_BASE_URL=...` 不会有任何效果。实测过：分别用默认值和自定义值各 build 一次，`routes-manifest.json` 里的目标地址确实随构建期的值变化，运行期改环境变量不生效。这一点在文档里专门标注，避免用户以为像 apps/api 的环境变量一样运行时可改。

**已完成的产出**：`apps/api/Dockerfile`、`apps/web/Dockerfile`（均需从仓库根目录构建，`docker build -f apps/api/Dockerfile .`）、根目录 `.dockerignore`、`docker-compose.yml` 新增 `api`/`web` 两个服务（`api` 容器启动时先跑幂等的 `db:migrate` 再 `start`；`web` 构建时通过 `args.API_BASE_URL: http://api:3000` 指向 `api` 服务的 compose 内网地址）、根目录 `.env.example`（docker-compose 用，跟 `apps/api/.env.example` 分开）、`docs/deployment.md` + `docs/deployment.en.md`（环境变量清单、密钥管理建议、`models.config.json` 的挂载方式、Railway 部署教程）。

**真实 Docker 验证（同日，用户中途装好 Docker Desktop 后补跑）**：实现阶段先在沙盒环境（当时没有 Docker）里用上述本地文件拷贝模拟排掉了两个最高风险的假设，随后用户安装好 Docker 之后，在同一个会话里补跑了真正的验证：

- `docker build -f apps/api/Dockerfile .` / `docker build -f apps/web/Dockerfile .` 均一次构建成功。
- 本地已有 Homebrew Postgres 占用 5432 端口，跟 compose 里的 `postgres` 服务冲突——临时 `brew services stop postgresql@16`，验证完之后 `brew services start postgresql@16` 恢复，没有影响其他数据。
- `docker-compose.yml` 里 `ENCRYPTION_KEY: ${ENCRYPTION_KEY:?错误提示...}` 这行在准备 `.env` 后跑 `docker compose config` 时直接报了 YAML 解析错误（`mapping values are not allowed in this context`）——原因是错误提示文案里有一个裸的英文冒号加空格（`generate with: openssl...`），没加引号的 YAML 纯量字符串里冒号加空格会被解析成新的 mapping，这是写 `${VAR:?message}` 这种 fallback 语法时一个容易踩的 YAML 坑。修复：把整个值加上双引号。
- `docker compose up --build` 三个服务全部正常启动，`api` 容器启动日志确认 6 个迁移文件全部从零应用成功。
- 通过 `web` 容器的 rewrite 代理（`localhost:3001/api/...`，验证的是真实的容器间网络，不是绕过代理直连 `api` 容器）走了完整流程：创建会话 → 提交一个 BYOK run（假 API key + `save:true`）→ 直接 `docker compose exec postgres psql` 查询确认加密列不含明文 key → 提交一个用默认 mock 模型的 run 并等到 `completed`，`GET /result` 返回正确的 `final_answer`/`cost`。假 key 的 run 收到真实 OpenAI 返回的 401（证明容器出网正常），被正确标记为 `failed` 且没有让 `api` 容器崩溃或影响其他请求。
- **顺带验证了一个此前只是推测、这次真机确认的细节**：`middleware/workspace.ts` 签发的 workspace cookie 在 `NODE_ENV=production`（Dockerfile 里设置）下带 `Secure` 属性——这是本来就有的行为，但这次用真实容器确认了 `Set-Cookie` 头确实带这个属性，意味着生产环境不走 HTTPS 的话真实浏览器根本不会保留这个 cookie（`curl` 因为不遵守 `Secure` 语义，不会暴露这个问题）。已经在 `docs/deployment.md` 里加了单独一节说明。
- 验证完毕清理了所有测试容器/卷/镜像，`.env`（含临时生成的真实密钥）和 cookie 临时文件也一并删除，没有留下痕迹。
...
### M5.5 分享链接 — 实际结果（已完成，2026-07-05，M5 到此全部完成）

按设计落地，`0007_share_token.sql` 给 `runs` 加 `share_token`（唯一、可空）；`POST /api/runs/:id/share`（幂等——已分享过直接返回同一个 token，不会每点一次生成一个新的）/`DELETE /api/runs/:id/share`（撤销）复用 `runs.ts` 已有的 workspace 归属校验和 409（running）/422（failed）语义；新增独立的 `routes/share.ts` 承载真正公开的 `GET /api/share/:token`——**特意不放进 `runs.ts`**，因为那个文件里所有路由都靠同一个 `onRequest` 钩子解析 workspace，公开路由单独成文件能让人一眼看出这是唯一的例外，不用担心以后有人不小心把 workspace 校验也加到这条路由上。

**安全性上一个提前想清楚、写代码前就验证过的点**：现有 `GET /result`（认证态）的实现方式本来就是"挑字段拼返回值"（`{runId, question, mode, status, ...result}`），而不是把整行 DB record 展开（`...run`）——`run.workspaceId`、`model_config`（BYOK 场景可能包含相关信息）从来没有被这条已有路径吐出过。公开的分享接口原样复用同一种"挑字段"写法，天然不会泄漏这些字段，不需要额外写一层过滤逻辑。写了一个测试直接断言 `JSON.stringify(body)` 不包含 token 本身、且 `workspaceId`/`shareToken` 字段是 `undefined`，把这个"结构上就安全"的假设钉死成回归测试，而不是只靠代码审查判断。

**前端一个原计划没预料到、但实现后发现必须处理的问题**：M3 时期的根 `layout.tsx` 把侧边栏（"+ New conversation"、会话列表）写死在最外层，所有页面都会带着这套 chrome。分享页面如果直接复用这个根 layout，陌生访客点开分享链接会看到一个跟自己毫不相关的"新建会话"按钮和空会话列表——不完全违反"不出现任何需要 workspace 身份的操作"这条验收标准的字面意思（"新建会话"本身不是分享页面自己发起的操作），但明显不是"干净的只读分享页"该有的样子，也确实会因为访问这个页面本身而给访客发一个新 workspace cookie（根 layout 的会话列表要 fetch `/api/conversations`，这个请求会触发 `resolveWorkspace`）。改用 Next.js App Router 的 route group：把侧边栏 chrome 从根 `layout.tsx` 挪到新增的 `app/(app)/layout.tsx`，原来的页面（`page.tsx`、`conversations/...`）一并挪进 `(app)/` 分组（route group 不影响实际 URL），根 `layout.tsx` 只剩下 `<html><body>{children}</body></html>`。`app/share/[token]` 留在 `(app)/` 分组之外，只继承那个空的根 layout——访问分享页面时完全不会请求 `/api/conversations`，也就完全不会触发 workspace cookie 签发。

**复用已完成 M3 组件带来的额外收益**：把 `RunPageClient.tsx` 里"完成态"那一块（`FinalAnswerPanel`/`ConsensusPanel`/`DiscussionProcess`/`PlanDocumentView`）抽成独立的 `RunResultView` 组件，认证页和分享页共用——这意味着分享页自动继承 M0 协议要求的透明性兜底（`ConsensusPanel` 的"展开查看原始 claims"追溯功能），不用重新决定"分享页要不要也支持追溯"这个问题，因为组件本身就是同一份。

**真实验证（2026-07-05，本地开发服务器 + 浏览器，非 mock 测试）**：
- 后端：`apps/api` 新增 4 个测试（全部打真实 Postgres）——分享/复用同一 token/撤销/撤销后 404 的完整流程；跨 workspace 分享或撤销返回 404；`running`（用 `MockProvider({latencyMs: 300})` 制造出确定性的"还在跑"窗口）和 `failed`（复用 M2 已有的 `failModelIds` 手法）状态下分享请求分别返回 409/422。全 workspace 8 个包 `npm run test` 223 个测试通过（`apps/api` 62 个，M5.5 新增净 4 个），零回归；额外用一个全新数据库验证了 7 个迁移文件从零按序应用成功。
- 前端：起了真实的 `apps/api`/`apps/web` 开发服务器，浏览器里走了一遍完整流程——创建会话、提交 quick 模式 run、完成后点击"Share"生成链接、复制/查看链接、导航到 `/share/:token`（确认没有侧边栏、没有"+ New conversation"，`document.cookie` 全程为空）、验证 Final answer/共识面板（含"show sources"追溯展开）/Discussion process 折叠区跟认证态页面渲染的内容完全一致、点击"Unshare"后原链接立刻显示"invalid or has been revoked"。全程浏览器控制台无报错。
- **验证过程中一个跟这次功能本身无关、但值得记录的工具层面发现**：这次会话里 `preview_click` 工具在这个环境下没有可靠地触发 React 的受控组件事件（点击按钮/radio 后 React state 没有更新，尽管点击命中的确实是目标 DOM 节点本身，`document.elementFromPoint` 确认过没有被其他元素遮挡）；`preview_fill`（用于文本框）本身工作正常。改用 `preview_eval` 执行 `element.click()`（原生 DOM click，而不是 CDP 层面模拟的鼠标事件）之后所有交互全部正常触发 React 状态更新。不确定是这个环境/浏览器版本的特有问题还是普遍现象，先记录下来，下次如果又遇到 `preview_click` "点了但页面状态没变"，可以直接跳到这个 workaround，不用重新排查一遍。

至此 M5（项目收尾与生产就绪）五个子项——成本熔断、CI、限流与数据清理、部署文档/Dockerfile、分享链接——全部完成。

## M6：能力扩展（流式输出 / 工具调用 / 多模态输入 / 自定义 JSON 输出）— M6.1–M6.6 已完成

M5 把项目带到"生产就绪"，M6 是在稳定的六阶段协议之上叠加四类新能力。完整的可行性分析、改动范围、对现有协议的影响和逐项取舍见 [streaming-tools-multimodal-json.md](streaming-tools-multimodal-json.md)——本节只定里程碑范围、实施顺序和验收方向，设计细节不在这里重复（跟 `protocol.md`/`roadmap.md` 一贯的分工一致：这里讲"做什么、按什么顺序、验收什么"，那份文档讲"为什么这么设计"）。

**贯穿 M6 的三条硬约束**（任何子项都不得违反，否则宁可不做）：
1. **不动六阶段协议 schema**：`FinalAnswerSchema`/`PlanDocumentSchema` 等保持稳定，前端共识/分歧/追溯/分享页的数据源不能因为新能力而漂移——多模态只在 propose 入口加输入类型，自定义 JSON 只在 compose 之后加一层，都是加法不是改法。
2. **不绕过"输出必须过 schema 校验才可信"这条纪律**：流式预览、工具结果、用户自定义 JSON 一律在后端校验一次再用，不把未校验的半成品推给前端（这是 M1/v0.2/M2 那三次"mock 太听话导致盲区"教训的同一条原则）。
3. **新增的模型调用必须纳入 M5.1 成本熔断**：自定义 JSON 格式化多一次调用、工具调用多一到两次往返，usage 都要计入跨阶段累加，否则会低估花费、破坏成本上限的准确性。

按可行性分析第 5 节的推荐顺序拆成 6 个子项（顺序即优先级）：

### M6.1 用户自定义 JSON 输出（先钉产品语义，最高优先级）
- **双层输出**：保留内部 `FinalAnswer`/`PlanDocument` 供共识/追溯/UI/分享，另加一个 compose 之后的 `FormatToUserJson` 层，把最终结果转成用户在发起 run 时指定的 JSON shape（`userOutput`）；**不拿用户 schema 替换内部 schema**。
- 新增 schema-agnostic 的 `callJsonSchema()`（Ajv 校验用户运行时 JSON Schema + repair 重试，复用 `structured.ts` 已有的 `extractJson`）；`ajv` 是这条路径唯一新增依赖。
- API `POST /runs` 加可选 `outputFormat`；`run_results` 加 `output_format`/`user_output` 两列；这次额外调用计入成本熔断。第一版只支持常见 JSON Schema 子集，不上 `$ref`/递归。
- 影响面：API + orchestrator 最终格式化层 + DB + web。**验收**：用户不填 `outputFormat` 时行为完全不变；填了则 `GET /result` 旁挂一份通过 Ajv 校验的 `userOutput`，repair 失败有明确降级。设计见文档第 1 节。

**已完成（2026-07-08）**。开工前拍板了文档第 6 节里跟 M6.1 相关的两个待决问题：第一版只支持用户直接写 JSON Schema（不做"给示例推断 schema"）；`userOutput` 附加在现有 `GET /result` 里，不另开轻量 endpoint。

- `packages/model-adapters` 新增 `callJsonSchema()`（`json-schema-output.ts`，复用 `structured.ts` 导出的 `extractJson`，Ajv 校验 + repair 重试，逻辑与 `callStructured` 对称）和 `validateOutputFormatSchema()`（`json-schema-validate.ts`，v1 子集白名单 + 深度 ≤6 + 大小 ≤16KB，显式拒绝 `$ref`）；`ajv` 是这条路径上唯一新增依赖。
- `packages/prompts` 新增 `buildFormatUserOutputPrompt`：把已经算好的内部 `FinalAnswer`/`PlanDocument` 和用户 schema 一起交给协调者模型，明确要求"只重排格式、不新增事实、不裁判 disputed"。
- `packages/orchestrator`：`DeliberationInput.outputFormat` / `DeliberationResult.{outputFormat,userOutput,userOutputError}`；standard/quick 在 compose 之后、planning 在 `planDocument` 组装之后各插入一次可选的 `FormatToUserJson` 调用（`tryFormatUserOutput` helper，两条路径复用），失败时只记录 `userOutputError`、不让整个 run 失败，usage 通过既有 `recordUsage` 折入 M5.1 成本累加，事件复用 `phase_started`/`phase_completed` + `data.step="format_user_output"`，没有新增 `Phase`/`RunEventType`。
- `apps/api`：`CreateRunBody.outputFormat` + 路由层 `validateOutputFormatSchema` 前置校验（400 快速失败）；迁移 `0008_output_format.sql` 给 `run_results` 加 `output_format`/`user_output`/`user_output_error` 三列（均可空）；`results-repo.ts` 相应读写。
- `apps/web`：`QuestionForm` 加一个默认折叠的"Custom JSON output (optional)"文本框（本地 `JSON.parse` 粗校验，服务端子集校验兜底）；新增 `CustomJsonOutputPanel`（带复制按钮）；`userOutputError` 时显示"main result 不受影响"的提示，不隐藏也不吓唬用户。
- **验证**：全仓库 8 个包 `npm run build`（依赖顺序）+ `npm run test` 共 252 个测试全绿（`model-adapters` 新增 13 个、`orchestrator` 新增 5 个、`apps/api` 新增 3 个，覆盖：省略 `outputFormat` 时行为不变、有效 schema 下 `userOutput` 通过校验、`$ref`/超限 schema 在创建 run 前就 400、repair 耗尽后降级为 `userOutputError` 且主结果不受影响、格式化调用的 usage 折入成本合计）。真实浏览器验证：起了本地 `apps/api`（`MockProvider`）+ `apps/web` 开发服务器，走了一遍完整创建 run 表单（含展开自定义 JSON 输出文本框）、提交、轮询、结果页；因为 `MockProvider` 不认识 `format_user_output` 这个 step（只认六个协议阶段），实际触发的是 provider 直接报错→`userOutputError` 降级路径（而不是"JSON 格式不对→repair"路径），确认了页面正确显示琥珀色提示且 Final answer/共识面板与不带自定义格式时完全一致；另外直接用 `fetch` 对 API 验证了 `$ref` schema 返回 400、不带 `outputFormat` 的 run 结果里完全没有 `outputFormat`/`userOutput`/`userOutputError` 字段（跟功能上线前的响应逐字节一致）。从 `0001` 到 `0008` 在全新数据库上顺序应用成功。真正的"用户 schema 校验通过、`userOutput` 成功生成"路径由 `model-adapters`/`orchestrator`/`apps/api` 三层的自动化测试（用一个识别 `format_user_output` step 的测试专用 provider）覆盖，浏览器里受限于 `MockProvider` 没有走到这条分支——这是本地无真实模型 key 时的已知限制，接入真实模型后应补一次端到端确认。

### M6.2 per-model 细粒度进度事件（投入产出比最高）
- `FanoutOptions` 加可选 `onSettled` 回调，`fanOutWithQuorum` 每个模型 settle 即回调（不再等 `Promise.all` 整体完成）；orchestrator 调用点各加一行，emit 新 `RunEventType` `model_responded {modelId, ok, latencyMs}`。
- 量级是 phase×模型，走现有持久化+广播路径即可，无写放大。**验收**：一个 phase 内先答完的模型能被前端立刻看到，不必等整个 phase settle。设计见文档 2.1。

**已完成（2026-07-09）**。开工前核实了设计文档里"orchestrator 六处调用点"的说法——实际是 **8 处**（propose/critique/revise/vote 四个 fan-out 阶段 × `runStandardOrQuickDeliberation`/`runTopicDeliberation` 两条代码路径），已在此处更正；normalize/compose 是单次协调者调用，本来就不在范围内。

- `packages/model-adapters`：`FanoutResult`（成功/失败两种）新增 `latencyMs` 字段，在 `fanOutWithQuorum` 内部围绕每个模型自己的 `withRetry(withTimeout(...))` 计时（含重试耗时）；`FanoutOptions` 新增可选 `onSettled?: (result, index, total) => void`，在每个模型 resolve/catch 后立即调用，不等整体 `Promise.all`。
- `packages/orchestrator`：`RunEventType` 新增 `"model_responded"`；新增一个共享的 `reportModelResponded(emit, phase)` helper 避免 8 处调用点重复拼装事件体，emit 的 `data` 为 `{modelId, ok, latencyMs, total, error?}`（失败时才带 `error`）。`runTopicDeliberation` 里事件复用局部 `emit` 闭包自动带上的 `topicId`，无需特殊处理。
- `apps/api`：无需改动——`run-service.ts`/`events-repo.ts`/`broadcaster.ts` 三层持久化+广播路径对事件类型完全无感知，新事件类型自动走通。
- `apps/web`：`SSE_EVENT_TYPES` 加入 `"model_responded"`（否则 `EventSource` 不会为这个类型注册监听器，事件会被静默丢弃）；`progress.ts` 的 `FlatProgress`/`TopicProgress` 新增 `modelProgress: Partial<Record<Phase, {responded, total}>>`，`phase_started` 时重置、`model_responded` 时累加；`PhaseStepList` 在 `in_progress` 的 phase 旁加一个 `{已响应}/{总数}` 的小计数器，`PhaseProgress`/`PlanningPhaseProgress` 分别接入 flat/per-topic 的 `modelProgress`。
- `apps/cli`：顺手给 `model_responded` 加了一行日志（`- modelId ok/failed in Nms`），非必需但改动成本接近零。
- **验证**：`packages/model-adapters`（4 个新测试：结算顺序早于慢模型、成功/失败都触发、index/total 覆盖每个位置各一次、`latencyMs` 在超时场景下的合理边界）、`packages/orchestrator`（4 个新测试：standard 模式 12 个事件且不出现在 normalize/compose 上、quick 模式仅 propose 的 3 个、单模型失败时该模型事件 `ok:false` 带 `error`、planning 模式 2 主题 × 4 阶段 × 3 模型共 24 个事件且 `topicId` 全部正确）、`apps/web`（4 个新测试：flat 模式累加、phase 重启时重置、planning 模式按主题独立累加）全部通过，未破坏任何既有测试。真实浏览器验证：起了本地 `apps/api`（`MockProvider`）+ `apps/web`，直接查询 Postgres `run_events` 表确认一次真实 standard 模式 run 里 `model_responded` 精确出现在 propose/critique/revise/vote 四个阶段、不出现在 normalize/compose；又在页面里用 `fetch`+`EventSource` 直接对新建的 run 订阅 SSE，确认浏览器端 `addEventListener("model_responded", ...)` 能实时收到与 DB 一致的事件（含 planning 模式下每个事件都带正确的 `topicId`）。因为 `MockProvider` 单次调用只需几毫秒，标准页面截图来不及捕捉到"进行中"状态下的计数器渲染画面——这是本地无真实模型延迟时的已知限制，接入真实模型（单次调用通常几秒到几十秒）后应补一次可视确认。

### M6.3 claim/item 级渐进式解析（与 M6.4 共享 provider 流式接口，建议一起设计接口、分两次落地）
- 后端做"增量数组元素提取"状态机（新增 `packages/model-adapters/src/streaming-json.ts`，只跟踪转义状态和括号深度，不是完整 JSON 解析器），只推送语法完整且过对应子 schema（`ClaimSchema`/`ReviewSchema`/`RevisionSchema`/`BallotSchema`/`CandidateClaimSchema`）校验的条目；**不把半截 JSON 甩给前端做容错解析**。
- 覆盖 propose/critique/revise/vote/normalize 五个结构化阶段，复用现有子 schema，不新建 schema。新增 `item_progress` 事件，走标准持久化。权威数据仍来自流结束后 `callStructured` 的全量校验。**验收**：五个阶段能逐条 append 卡片；repair 重试（新 `attempt`）时前端清空重画。设计见文档 2.2。

**已完成（2026-07-09，与 M6.4 一起设计、分两步落地——本项是 Step 1）**。

- `packages/model-adapters/src/streaming-json.ts`（新文件）：`createArrayItemWatcher(targetField, onItem)` 状态机——只用一个嵌套深度计数器 + 字符串转义状态，不是完整 JSON 解析器；**关键字段位置感知**而非对 `"claims":` 做裸字符串搜索（JSON-mode 模型不保证字段顺序，且该字面量可能巧合出现在别的字符串值里），只在深度为 1 且紧跟在 `{`/`,` 后面的字符串才当作候选 key。"条目完整"被精确定义为"让深度从 arrayDepth+1 回落到 arrayDepth 的那个 `}`"，因此条目内部的嵌套数组（如 `Claim.conditions: string[]`）不会误触发。`createValidatedArrayItemWatcher` 在此之上叠加一层 zod 校验，解析/校验失败静默丢弃（这只是预览，权威结果仍来自流结束后 `callStructured` 的全量校验）。
- `ModelProvider` 新增可选 `completeStream?(config, request, onDelta, opts?)`（本节与 M6.4 共享设计的同一接口，一次性加完）；`OpenAICompatibleProvider` 实现为 `stream:true` + SSE 逐帧解析 + `AbortController`（绑定 `opts.timeoutMs`，见下方"发现的真实问题"）；`MockProvider` 新增**严格 opt-in** 的 `streaming`/`streamChunkSize` 选项（默认不挂载 `completeStream` 方法本身，而不是挂载一个空实现，这样每个调用点的 `provider.completeStream` 判断都会正确落回 `complete()`，不破坏任何既有测试对调用次数的假设）。
- `packages/orchestrator`：`RunEventType` 新增 `"item_progress"`；orchestrator 内部（非导出）的 `structuredCall` 包装函数新增可选第 6 个参数 `StreamHooks`（`onDelta(delta, attempt)` + `timeoutMs`），内部判断 `provider.completeStream && stream?.onDelta` 决定走流式还是原来的 `complete()`；`packages/model-adapters` 导出的 `callStructured` 闭包签名新增 `attempt` 形参（全仓库唯一调用方就是这个 orchestrator 包装函数，零风险改动）。新增 `ARRAY_STREAM_CONFIG` 静态表（propose→claims/critique→reviews/revise→revisions/vote→votes/normalize→candidate_claims）+ `makeItemStreamHandler` 工厂，在 10 处（5 阶段 × standard-or-quick 与 planning 两条代码路径）`structuredCall` 调用点各加一行 hook；outline/compose/section_compose 三处不改动。
- **发现的真实问题（设计阶段未预料到）**：`fanOutWithQuorum` 的 `withRetry(() => withTimeout(...))` 只包住 8 处 fan-out 调用（不含 normalize/compose/outline/section_compose），且 `withTimeout` 用 `Promise.race` 从不真正取消底层调用——如果流式调用超时后 `withRetry` 又发起一次重试，前一次尝试可能还在后台跑、继续吐 delta。**结论**：`completeStream` 内建的 `AbortController`（绑定同一个 `timeoutMs`）足以把这个缝隙基本关上，不需要再给 `fanout.ts`/`resilience.ts` 加一个"抑制 delta 直到最后一次尝试"的标志位——因为 `item_progress` 的 `index === 0` 本身已经能同时覆盖"schema repair 重试"和"fan-out 网络重试"两种情况（两者都会让整个构建 `onDelta` 的箭头函数从头重新执行，从而重置 `index`），前端只需一条规则：收到某个 `(phase, modelId)` 组合的 `index === 0` 事件时清空重画，不需要区分是哪种重试。
- `apps/api`：**无需改动**——`run-service.ts`/`events-repo.ts`/`broadcaster.ts` 三层对事件类型完全无感知，`item_progress` 复用和 `model_responded` 一样的持久化+广播路径（量级是"phase 数组长度 × 模型数"，同一数量级，不需要旁路）。
- `apps/web`：`SSE_EVENT_TYPES` 加 `"item_progress"`；`progress.ts` 新增 `itemProgress: Partial<Record<Phase, PhaseItemProgress>>`（`PhaseItemProgress` 按 modelId 分组），`phase_started` 时重置、`index===0` 时替换（不是追加）、`index>0` 时追加——这条"清空重画"规则完全在前端纯函数里实现，不依赖后端任何额外打标。新增 5 个小型展示组件（`ProposalClaimPreview`/`ReviewPreview`/`RevisionPreview`/`BallotPreview`/`CandidateClaimPreview`，都是从 `DiscussionProcess.tsx` 已有内联 JSX 里原样抽取出来的）+ `LivePhaseItems`（按 `arrayField` 分发到对应展示组件，只在该 phase `status==="in_progress"` 时渲染）。`CandidateClaimPreview` 刻意区别于已有的 `CandidateClaimItem.tsx`——后者需要整个 run 的 `proposals`/`revisions` 来解析溯源，跑到一半时还没有这些数据。`RunPageClient.tsx`/`PlanningPhaseProgress.tsx` 在原有 `PhaseProgress`/`PlanningPhaseProgress` 后面插入 `<LivePhaseItems>`——这是此前完全不存在的"运行中局部结果展示"位置，此前运行中只显示阶段状态点，完成后才整体切换到 `RunResultView`。
- **验证**：`packages/model-adapters`（`streaming-json.test.ts` 新增 16 个测试：完整数组一次性 feed、逐字符 feed、任意切分点 feed 三种方式结果一致、转义反斜杠恰好切在 feed 边界、条目内嵌套数组不误判、字段名任意顺序都能找到、"文本里恰好出现 `\"claims\":`"不会误触发（证明是关键字位置感知而非裸字符串搜索）、字段不存在时不触发、schema 校验失败静默丢弃；另加 `MockProvider`/`OpenAICompatibleProvider`/`RoutingProvider` 的流式与超时中止测试）、`packages/orchestrator`（新增 4 个测试：standard 模式 `item_progress` 只出现在五个结构化阶段且内容与最终结果一致、quick 模式只有 propose+normalize、repair 重试时 `attempt` 能区分被放弃的第一次尝试与最终采纳的一次、planning 模式每个事件都带正确 `topicId`）、`apps/web`（`progress.ts` 新增 5 个测试）全部通过，未破坏任何既有测试。**真实浏览器验证**：临时把本地 `apps/api` 的零配置兜底 `MockProvider` 打开 `streaming:true`（验证完之后已改回默认关闭，未作为永久默认变更——这是一次纯粹为了可视化验证而做的临时切换，是否要默认打开留给用户决定），发现并修复了一个**设计阶段遗漏的真实 bug**：`apps/api` 的 `buildRunProvider` 总是把每个模型的 provider 包一层 `RoutingProvider`，而 `RoutingProvider` 原本完全没有转发 `completeStream`——这意味着哪怕接入真实的、已经实现流式的 `OpenAICompatibleProvider`，流式在生产环境里也永远不会被触发。修复：给 `RoutingProvider` 加一个**总是挂载**的 `completeStream`（因为不同模型标签可能路由到不同底层 provider，构造时无法预先知道哪些支持流式），按 `config.id` 查到对应路由后，若该路由的 provider 有 `completeStream` 就转发，否则退化为普通 `complete()` 调用（`onDelta` 不会被调用，但请求仍然正常成功）——纯加法，不影响任何走非流式路径的现有测试。用 `fetch`+`EventSource` 直接对新建 run 订阅 SSE，确认 `item_progress` 事件精确出现在 propose/critique/revise/vote/normalize 五个阶段（standard 模式 3 模型共 48 条，planning 模式 2 主题共 96 条），且 planning 模式每条都带正确 `topicId`。因为 `MockProvider` 单次调用只需几毫秒，标准页面截图来不及捕捉"进行中"状态下逐条 append 的画面——与 M6.2 相同的已知限制，接入真实模型后应补一次可视确认。

### M6.4 compose 阶段 token 级流式
- 给 `ModelProvider` 加可选 `completeStream?(config, request, onDelta)`（与 M6.3 共享同一接口，`MockProvider` 等不实现也兼容）；仅对 compose/section-compose 的自由文本 prose 做打字机效果（结构化阶段不适用——没有 prose 字段可流）。
- `token` 事件**只广播不落库**（`RunBroadcaster` 加 `publishEphemeral`，跳过 `appendRunEvent`），避免 `run_events` 写放大；断线重连靠 `phase_completed`（已含完整文本）兜底。**验收**：compose 打字机流式，`run_events` 写入量不随 token 数暴涨。设计见文档 2.3。

**已完成（2026-07-09）**。开工前确认了一个设计文档没直接回答、但会影响范围的问题：planning 模式的 section-compose 是否也要做——采纳的决定是**两种模式都做**（standard/quick 的单次 compose + planning 每个 topic 独立一路 section-compose 打字机，多路并行渲染），因为后端改动量在两种情况下完全对称（只是多传一个 `topicId` 字段），额外成本纯粹在前端"同时渲染 N 路打字机"这一侧。

- `packages/model-adapters/src/streaming-json.ts` 新增第二个导出 `createStringFieldWatcher(targetField, onChars)`：复用 M6.3 状态机同一套"关键字位置感知"寻找 `"targetField":`，找到后不是进数组模式而是进入字符串内容中继模式——解码转义序列（`\n \t \r \b \f \" \\ \uXXXX`，转义序列本身跨 feed() 边界也能正确处理），把解码后的原始字符实时中继出去，直到该字段自己的收尾引号为止。这里**不做 schema 校验**——半截 prose 字符串本来就没法有意义地校验，最终完整文本仍然走 `callStructured` 的全量校验。**设计阶段遗漏、实现时修正的一点**：最初设想"直接转发 compose 的原始 token delta"，但 `final_answer`/`section_answer` 只是响应对象里的一个字段，原始 provider delta 是 JSON 语法（引号、其它字段名、花括号）和 prose 交织在一起的——直接转发等于把 M6.3 特意要避免的"把半截 JSON 甩给前端"问题原样复刻了一遍到 M6.4；`createStringFieldWatcher` 就是为了堵上这个口子而不是简单转发。
- `packages/orchestrator`：`RunEventType` 新增 `"token"`；新增 `makeTokenStreamHandler` 工厂（与 M6.3 的 `makeItemStreamHandler` 对称，同样按 `attempt` 重置）；compose（standard/quick，1 处）和 section_compose（planning，每个 topic 一次并行调用）两个调用点各加 6 个参数；**section_compose 用的是 `runPlanningDeliberation` 顶层的 `emit`（不像 `runTopicDeliberation` 会自动把 `topicId` 塞进 `data`），所以必须显式在 `extraData` 里传 `{topicId: topicResult.topic.topic_id}`**——这是实现时才发现的细节，容易漏掉。compose/section_compose 本来就不经过 `fanOutWithQuorum`，所以 M6.3 那个"网络重试与流式的竞态"问题在这里完全不存在，`timeoutMs` 纯粹是兜底防挂死。
- `apps/api`：`events-repo.ts` 的 `extractTopicId` 由私有改为导出；`broadcaster.ts` 新增 `writeSseEventEphemeral`（跟 `writeSseEvent`同样格式但**不写 `id:` 行**——ephemeral 事件没有 DB 分配的 seq，绝不能推进客户端断线重连的 `Last-Event-ID` 游标）和 `RunBroadcaster.publishEphemeral`；`run-service.ts` 的 `onEvent` 按 `event.type==="token"` 分支，**仍然挂在同一个 `eventChain` 上**（只是跳过 `appendRunEvent`/`seq` 计数器）——因为 Node 单线程、每次 `emit()` 都会在下一次 `emit()` 执行前同步重新赋值 `eventChain`，这样 ephemeral 事件和持久化事件之间的相对顺序不需要额外同步就能保持正确。
- `apps/web`：`SSE_EVENT_TYPES` 加 `"token"`；`RunEventMessage.seq` 类型改为 `number | null`。**性能取舍**：没有让 token 事件进入 `useRunEvents` 现有的扁平 `events` 数组（长时间 compose 流会让数组无限增长，且 `deriveRunProgress` 每次都要重新扫一遍），也没有开第二条 `EventSource`（会让服务端订阅者数量翻倍，重复消耗同一条流）——而是给 `useRunEvents` 加了第二块状态 `composeText: Record<string, string>`（按 `topicId` 或 `"_root"` 分组），累加逻辑抽成 `run-events.ts` 里的纯函数 `accumulateComposeText`（这个仓库测试基础设施没有 jsdom/hook 测试环境，所以刻意把逻辑抽成纯函数单测，而不是引入新的 hook 测试基础设施）。`FinalAnswerPanel.tsx` **不需要新组件**——它本来就只依赖 `{title?, text}`，运行中直接喂一个不断增长的字符串即可；planning 模式每个正在 compose 的 topic 各渲染一个独立实例。
- **验证**：`packages/model-adapters`（`streaming-json.test.ts` 新增 7 个测试：任意切分点下精确重建原字符串、`\uXXXX` 转义恰好切在边界、收尾引号处精确停止且不泄漏后续兄弟字段、字段任意顺序）、`packages/orchestrator`（新增 3 个测试：standard 模式 `token` 只出现在 compose 且拼接后与 `final_answer` 逐字节一致、不支持流式时零 `token` 事件、planning 模式每个 topic 的 `token` 拼接后与各自 `section_answer` 逐字节一致）、`apps/api`（新增 `run-service.test.ts`，2 个真实 Postgres 集成测试：`token` 事件从不写入 `run_events` 表但 phase/model_responded/item_progress 都正常持久化、通过真实 SSE 订阅确认 `token` 事件在 wire 上与持久化事件保持正确相对顺序；`broadcaster.test.ts` 新增 5 个测试确认 `publishEphemeral` 从不写 `id:` 行）、`apps/web`（新增 `run-events.test.ts` 3 个测试：`accumulateComposeText` 按 key 独立累加、不修改前一个状态对象）全部通过。**真实浏览器验证**：同样临时打开本地 `MockProvider` 的 `streaming:true`（验证后已改回默认关闭），用 `fetch`+`EventSource` 直接验证：standard 模式 374 个 `token` 事件拼接后与 `GET /result` 返回的 `final.final_answer` 完全一致；planning 模式 973 个 `token` 事件按 `topicId` 分组拼接后，两个 topic 各自与 `planDocument.sections[].section_answer` 完全一致；`token` 事件确认从不携带 `id:` 行（浏览器端 `EventSource.lastEventId` 在 token 帧之间保持上一个真实持久化事件的 seq 不变，这正是 SSE 规范下预期的"粘性"行为，用来保证断线重连游标始终锚定在最后一个可重放的真实事件上，不是 bug）。全仓库 `npm run build` + `npm test --workspaces` 共 311 个测试全部通过。因为 `MockProvider` 单次调用只需几毫秒，页面截图同样来不及捕捉"打字机效果"进行中的画面——与 M6.2/M6.3 相同的已知限制。

### M6.5 多模态输入（仅 propose 入口）
- `CompletionRequest.userPrompt` 从 `string` 扩成 `string | ContentPart[]`（`{type:"text"}`/`{type:"image_url"}`）；`OpenAICompatibleProvider` 按 OpenAI content-parts 格式组装。**只有"问题本身"这一个入口受影响**，critique/revise/normalize/vote/compose 消费的都是前序纯文本 claim，五个后续 phase 的 schema 不动。
- 第一版内联 base64（不引入对象存储，跟"无 Redis"同一种"能不加基础设施就不加"的取舍），配合现有匿名 workspace 30 天清理（`db/cleanup.ts`）过期；给单图/总量加硬上限，避免撑爆 Postgres 单行。**验收**：带图问题能跑完整六阶段，纯文本 phase 无回归。设计见文档第 4 节。

**已完成（2026-07-09）**。首版只支持 JPEG/PNG/WebP：最多 3 张、单张 5MB、总计 12MB。`POST /runs` 接受 `images: [{dataUrl}]`，服务端以严格的 canonical base64 data URL 校验并在 `runs.input_images` 保存，创建 run 路由单独提升到 18MiB body limit；原始图片不会出现在 run、result、SSE 或公开分享响应中，随 workspace 的 30 天级联清理删除。`CompletionRequest.userPrompt` 扩展为 OpenAI-compatible content parts，普通与流式请求都只在 propose 阶段发送图片；planning 的 outline 和所有后续阶段仍保持纯文本。Web 已提供选择、预览、删除与同等的客户端大小/MIME 校验，并提示用户选择支持视觉的模型。视觉不兼容按既有 quorum 路径降级，不新增模型能力注册表。

### M6.6 Tool calling（propose + critique，运行时可选、默认关闭）
- 单轮、内嵌在一次 `complete()` 内部；propose 和 critique 复用同一套机制，第一版只内置一个 `web_search`。`CompletionRequest` 加 `tools?`，`CompletionResult` 加 `toolCalls?`（仅审计展示，不进入后续阶段输入）。
- **两次往返的 usage 在 provider 内部合并成一个 `CompletionUsage`**（否则 M5.1 只看到最后一次往返、低估成本）；工具失败在 provider 内兜底（把错误当 tool 消息喂回模型继续，不让整个 `complete()` 失败）。
- `budget.ts` 加 `toolRoundTripAllowanceMs`，仅对启用 tools 的 run 的 propose/critique 生效，不全局改 `timeoutMs`。做成和 `costLimitUsd` 同一心智模型的运行时开关，默认关闭。风险是**同质化和成本**（多个模型查同一件事可能趋同、削弱 propose 多样性），不是协议冲突。设计见文档第 3 节。

**已完成（2026-07-09）**。首版支持两条 BYOK 原生路径：直接 OpenAI 用 Responses API 的 `web_search`，OpenRouter 用 Chat Completions 的 `openrouter:web_search` server tool；不新增独立搜索服务或密钥。`webSearch` 是 propose+critique 共用的单一运行时开关，默认关闭，只接受全由同一 provider（全 OpenAI 或全 OpenRouter）BYOK 模型组成的 run；服务端拒绝混入 legacy 或混合 provider 的请求，避免表面启用而实际只有部分模型获得检索能力。OpenAI 每次 Responses 调用最多允许一次原生搜索、`store:false`；OpenRouter 使用官方 `max_total_results` 上限，且两条路径都不跨模型缓存，保留独立提案。`toolRoundTripAllowanceMs` 只加到这两个 phase 的超时；OpenAI 的 $0.01/次 web-search 费用被并入 M5.1 成本估算，OpenRouter 继续采用其响应返回的真实 `usage.cost`。UI 明确提示兼容范围与成本/延迟影响。

### 开工前需要产品/协议先拍板的问题
设计文档第 6 节列了一组**不是纯工程、需要先定**的问题：自定义 JSON 是否也支持"给示例推断 schema"、`userOutput` 是否要独立轻量 endpoint、`item_progress` repair 时清空重画还是保留草稿、section-compose 是否也做 token 流式、`web_search` 结果是否跨模型共享缓存、propose/critique 的工具开关合一还是拆开、图片是否需要长期保留。建议在每个子项动手前逐条确认，避免做完再返工——这也是把 M6.1 排在最前的原因：它牵扯的产品语义最需要先钉死。
...
M0–M6 全部完成。M6.6 采用直接 OpenAI BYOK 的 Responses API 原生 web search：统一开关覆盖 propose 与 critique，不跨模型共享搜索结果，并将每次搜索的固定费用纳入成本估算。后续能力应从新的产品需求出发，而不是继续扩展当前 M6 范围。

其他已知的、不阻塞 M6 但值得记录的后续待办：
...
- ~~M2 的一个已知限制：API 进程在 run 执行中途重启会让该 run 永久卡在 `running` 状态~~——2026-07-05 部分解决：真正的跨重启可恢复性（断点续跑）仍然没做，也依然不打算做（复杂度不匹配当前规模）；但既然确定要部署到生产环境，"每次发布都会重启进程"从假设变成了确定会发生的事，所以补了 `apps/api/src/services/reconcile-runs.ts`——进程启动时（迁移已跑完、开始监听请求之前）把所有还挂在 `running` 状态的 run 直接判定为失败并补一条 `run_failed` 事件，让断线重连的客户端拿到一个真实的终止状态，而不是无限挂起。用真实 Postgres + 真实 Docker 开发服务器验证过：手动把一个已完成 run 的 `status` 改回 `running` 模拟"进程当时被杀掉"，重启 `apps/api` 后日志打出 `Reconciled 1 run(s)...`，浏览器刷新后正确显示 "Run failed: Interrupted by a server restart before completion."，而不是像修复前那样永远停在 "running"。详见 `docs/deployment.md`「部署/重启时正在跑的 run 会怎样」一节。
