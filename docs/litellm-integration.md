...
- 支持 `mmd/fusion` custom provider 外壳、`custom_provider_map` 配置示例、quick mode（Propose -> Normalize -> Compose）、quorum/partial、run-scoped claim id、structured repair、递归保护和 `return_trace` metadata。
- Python 行为测试覆盖 consensus、quorum、quick orchestrator、provider response shape 和递归保护；运行方式：`uv run --project python --extra test pytest`。

短期任务：

- 扩展 Python protocol port，补齐 standard mode 需要的 critique/revise/vote prompt builder 和 schema 行为测试。
- 用真实 LiteLLM extra 跑通 Proxy 调用 `model: mmd-fusion` 的 smoke test。
