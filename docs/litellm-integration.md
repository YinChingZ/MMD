...
- 支持 `mmd/fusion` custom provider 外壳、`custom_provider_map` 配置示例、quick mode（Propose -> Normalize -> Compose）、standard mode（完整六阶段）、quorum/partial、run-scoped claim id、structured repair、递归保护和 opt-in `return_trace` metadata。
...
1. **真实模型 Proxy smoke**：用真实 `analysis_models` / `coordinator_model` 配置跑通 `model: mmd-fusion`，确认真实 provider auth、timeout、repair retry 和 `mmd` trace 契约。
2. **Python planning mode**：迁移 outline、per-topic standard 并行、section compose 和 deterministic executive summary，保持 TypeScript CLI 行为一致。
3. **LiteLLM Router/callback 接入**：内部调用改成 Router 优先，明确 usage 聚合、callback/logging 中 trace 的落点。
4. **Upstream readiness**：整理异常类型、配置字段、测试命名和目录结构，使其更接近 LiteLLM PR 或 external provider package。

中期任务：

- 接入 LiteLLM Router 而不是直接请求 provider。
- 将异常、usage、callbacks 接入 LiteLLM 现有机制。
- 用真实模型配置补充 Proxy smoke 和失败/partial 场景。
- 迁移 planning mode。
