...
- 支持 `mmd/fusion` custom provider 外壳、`custom_provider_map` 配置示例、quick mode（Propose -> Normalize -> Compose）、standard mode（完整六阶段）、planning mode（Outline -> per-topic standard -> Section Compose）、quorum/partial、run-scoped claim id、structured repair、递归保护、Router-aware client、usage 聚合、opt-in `return_trace` metadata、opt-in `return_analysis` payload 和 opt-in `mmd_log_trace` callback payload。
...
1. **Advanced config**：补 preset、预算/熔断、max token 限制和 planning 相关保护。
2. **Upstream readiness**：整理异常类型、callback/logging upstream 形态、配置字段、测试命名和目录结构，使其更接近 LiteLLM PR 或 external provider package。
