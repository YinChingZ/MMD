...
- 准备 LiteLLM fork / upstream-shaped Python 目录；本 repo 内新增 Python 代码时也按 LiteLLM 风格组织。
- 用 Pydantic 重写 protocol schemas。
- 移植 `classifyCandidate` 和 quorum 测试。
- 把 `build*Prompt` 迁移为 Python prompt builder。
- 用 mock `acompletion` 跑通 quick mode。
- 写最小 LiteLLM config 示例，验证普通 OpenAI client 可以无感调用。
