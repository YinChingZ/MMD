...
- 支持 `mmd/fusion` custom provider 外壳、`custom_provider_map` 配置示例、quick mode（Propose -> Normalize -> Compose）、standard mode（完整六阶段）、planning mode（Outline -> per-topic standard -> Section Compose）、quorum/partial、run-scoped claim id、structured repair、递归保护和 opt-in `return_trace` metadata。
...
1. **LiteLLM callback/logging 接入**：Router-aware 内部调用和 usage 聚合已落地，下一步明确 callback/logging 中 trace 的落点。
2. **Advanced config**：补 preset、预算/熔断、max token 限制和 planning 相关保护。
3. **Upstream readiness**：整理异常类型、配置字段、测试命名和目录结构，使其更接近 LiteLLM PR 或 external provider package。

中期任务：

- 将异常和 callbacks 接入 LiteLLM 现有机制。
...
本地 Proxy smoke 已验证 LiteLLM response object 会保留顶层 provider-specific `mmd` metadata，第一版可用 `return_trace=true` 返回 `trace_version: 1` 的审计结构。但完整 trace 可能很大，生产环境还需要把同一结构接入 callback/logging，并考虑压缩、采样或仅记录摘要，避免普通 API 响应过重。
