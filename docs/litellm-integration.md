...
本文档描述 `litellm-integration` branch 的技术转向背景：MMD 不再按原路线优先建设独立 Backend API / Web MVP，而是把已经验证过的多模型协商协议改造成 LiteLLM 生态中的开源 Fusion-like 能力。

**后续开发主线以 [docs/fusion-replacement-mainline.md](fusion-replacement-mainline.md) 为准**：本 branch 的目标已经进一步明确为 LiteLLM 生态里的 open-source Fusion replacement，默认体验追求 Fusion 级可用性，MMD 协议作为更强的多模型整合内核。
...
| 参数 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `analysis_models` | `list[str]` | 必填 | 参与 panel 的模型。建议 1-8 个，与 Fusion 的心智模型接近。 |
| `coordinator_model` | `str` | `analysis_models[0]` | Normalize / Compose 使用的模型。 |
| `preset` | `cheap | balanced | strong` | 无 | 本地配置 preset：不展开具体厂商模型，只把已配置 panel 截断到 2/3/5 个，并设置默认 timeout。 |
| `mmd_mode` | `quick | standard | planning` | `quick` | 协商策略。 |
| `quorum_ratio` | `float` | `0.66` | 每个 fan-out 阶段最低响应比例。 |
| `per_model_timeout` | `float` | 根据 preset/mode | 单模型阶段超时；preset 默认为 30/60/120s，未设置 preset 时 quick/standard/planning 默认为 40/90/120s。 |
| `max_repair_attempts` | `int` | `2` | 结构化 JSON 修复重试次数。 |
| `max_topics` | `int` | `8` | planning mode 的最大 topic 数。 |
| `max_analysis_models` | `int` | `8` 或 preset 值 | 最大 panel 模型数，超过时保留前 N 个。 |
| `max_completion_tokens` | `int` | 无 | 转发给 panel/coordinator 调用。 |
| `temperature` | `float` | 无 | 转发给 panel 调用。 |
| `coordinator_temperature` | `float` | `0.1` | 转发给 Normalize / Compose / Outline 等 coordinator 调用。 |
| `reasoning` | `dict | str` | 无 | 透传给支持 reasoning config 的 provider。 |
| `tools` | `list[dict]` | `[]` | 透传给 panel 阶段的 LiteLLM/provider tools；第一版面向 provider-managed tools/web search。 |
| `tool_choice` | `str | dict` | 无 | 与 `tools` 一起透传。 |
| `max_tool_calls` | `int` | 无 | 与 `tools` 一起透传，作为 provider/LiteLLM 可识别的调用限制。 |
| `coordinator_tools_enabled` | `bool` | `false` | 是否也把 tools 透传给 outline/normalize/compose/section_compose 等 coordinator 阶段。 |
| `model_params` | `dict` | `{}` | 透传给所有底层模型调用的 provider-specific 参数。 |
| `analysis_model_params` | `dict` | `{}` | 仅透传给 panel 阶段的 provider-specific 参数。 |
| `coordinator_model_params` | `dict` | `{}` | 仅透传给 coordinator 阶段的 provider-specific 参数。 |
...
| `mmd_log_trace` | `bool` | `false` | 是否把瘦身审计 trace 发送到 callback/logging；默认关闭，不影响 HTTP response。 |

后续再考虑：

- provider key-aware preset expansion：按用户可用 provider 自动推荐/展开 panel models 和 coordinator。
- MMD 自建 function-calling tool loop：当前只做 provider-managed tools/web search 参数透传，不执行普通工具调用。
...
当 `return_trace=true` 时，当前 PoC 已验证 LiteLLM Proxy HTTP 响应会保留顶层 provider-specific `mmd` metadata，而不会破坏 `choices[].message.content`。该字段是显式 opt-in，默认 `return_trace=false` 时不返回完整 trace。当前契约固定为 `trace_version: 1` / `protocol: "mmd.v1"`：
...
当前 smoke 已覆盖 `/chat/completions` HTTP 路径：`python/scripts/proxy_smoke.py` 使用 scripted mock panel 启动本地 LiteLLM Proxy，并断言响应中存在 `mmd.trace_version === 1`。后续真实模型 smoke 应复用同一契约。默认不应把完整 trace 塞进 content，否则普通用户会得到过长、难读且 token 成本偏高的回答。

当 `return_analysis=true` 时，response 会额外返回顶层 `mmd_analysis`，而不是复用完整 `mmd` trace。第一版字段为 `analysis_version: 1` / `protocol: "mmd.analysis.v1"`，并包含 consensus_summary、disagreements、model_coverage、notable_unique_points、limitations；该 payload 从已产生的 consensus 数据确定性派生，不额外调用模型。
...
| structured output repair exhausted | 第一版映射为 `MMDProviderAPIError`（500，`code=mmd_api_error`，details 包含 cause）；后续可进一步贴 LiteLLM `APIError` 或 provider-specific bad response error。 |
| quorum not met | 第一版映射为 `MMDProviderQuorumError`（500，`code=mmd_quorum_not_met`，details 包含 phase/quorum/failures），message 包含 phase/respondent/required。 |
| invalid MMD config | 第一版映射为 `MMDProviderBadRequestError`（400，`code=mmd_bad_request`）。 |
| recursive MMD invocation | 第一版映射为 `MMDProviderBadRequestError`（400，`code=mmd_bad_request`）。 |

这些 typed MMD provider errors 不依赖 `litellm` extra，因此 Python 协议/行为测试可以在无 LiteLLM runtime 的环境下运行；后续 upstream/package 清理时再决定是否在 Proxy 入口转换为 LiteLLM 原生 `BadRequestError` / `APIError` 实例。
...
- 移植 planning mode（第一版已在本 repo 的 Python PoC 中完成）。
...
- 支持 `mmd/fusion` custom provider 外壳、`custom_provider_map` 配置示例、quick mode（Propose -> Normalize -> Compose）、standard mode（完整六阶段）、planning mode（Outline -> per-topic standard -> Section Compose）、quorum/partial、run-scoped claim id、structured repair、递归保护、Router-aware client、usage 聚合、advanced config 第一版、typed provider error mapping、provider-managed tools compatibility 第一版、opt-in `return_trace` metadata、opt-in `return_analysis` payload 和 opt-in `mmd_log_trace` callback payload。
- 已新增本地 LiteLLM Proxy HTTP smoke：`uv run --project python --extra proxy python python/scripts/proxy_smoke.py`，使用 scripted mock panel 验证 `/chat/completions` 能调用 `model: mmd-fusion-mock`，并断言 `mmd.trace_version === 1`，无需真实模型 key。
- 已新增并跑通真实模型 Proxy HTTP smoke：`uv run --project python --extra proxy python python/scripts/proxy_real_smoke.py`，通过 `MMD_SMOKE_ANALYSIS_MODELS` / `MMD_SMOKE_COORDINATOR_MODEL` 动态生成临时 LiteLLM 配置；未设置真实模型时输出 skipped，不提交任何 key。OpenRouter panel quick mode 已验证普通 content 和 `mmd.trace_version === 1` 契约。
- Python 行为测试覆盖 consensus、quorum、quick/standard/planning orchestrator、provider response shape 和递归保护；运行方式：`uv run --project python --extra test pytest`。

接下来的开发规划（按优先级）：

1. **Upstream readiness**：整理 callback/logging upstream 形态、配置字段、测试命名和目录结构，使其更接近 LiteLLM PR 或 external provider package。
2. **真实模型覆盖**：补 advanced config + tools compatibility 的 OpenRouter quick/standard/planning smoke。

中期任务：

- 将异常和 callbacks 进一步贴近 LiteLLM upstream 现有机制。
- 用真实模型配置补充失败/partial 场景。
- 完善 planning mode 的高级配置和真实模型验证。

后续任务：

- MMD 自建 function-calling tool loop / 更完整 web/search 体验。
- provider-specific token usage 细化与真实模型覆盖。
- 文档、示例、upstream PR。
- 如 upstream 暂不接受，发布为 LiteLLM-native external provider，而不是回到自建产品路线。

## 14. 风险

### 14.1 LiteLLM response metadata 体积与生产审计

本地 Proxy smoke 已验证 LiteLLM response object 会保留顶层 provider-specific `mmd` metadata，第一版可用 `return_trace=true` 返回 `trace_version: 1` 的审计结构。`mmd_log_trace=true` 已提供瘦身 callback/logging payload；生产形态还需要继续贴近 LiteLLM upstream 的 callback 配置、压缩、采样或摘要策略，避免普通日志过重。
...
OpenRouter Fusion 的实用性部分来自 panel/judge 可用 web_search/web_fetch。MMD 第一版已支持把 `tools` / `tool_choice` / `max_tool_calls` 透传给 provider-managed tools/web search，并在 trace/logging 中标记 tool availability；但还不自建普通 function-calling tool loop。research 类任务若依赖工具执行，应优先使用 provider/LiteLLM 已托管的 web/search 工具形态。
