...
这四项里，现有系统已经有**内部固定 JSON 协议输出**（各阶段 zod schema + repair 重试）；本文档里的 "JSON output" 特指**用户在发起 run 时指定最终输出 JSON 格式**，这一能力尚未实现。本文档是设计分析，不是已落地功能的说明（不同于 [protocol.md](protocol.md)/[deployment.md](deployment.md) 这类描述现状的文档）。
...
### 现状
`CompletionRequest` 没有 `tools` 字段，`OpenAICompatibleProvider` 的响应解析完全没处理 `message.tool_calls`。这是四项里唯一"字面上完全不存在"的能力。
...
1. **用户自定义 JSON output**——先把产品语义钉准：保留内部 `FinalAnswer`/`PlanDocument`，新增最终 `userOutput` 格式化层；provider 原生 JSON mode 只是后续可靠性增强。
2. **per-model 细粒度进度事件**——小，`fanOutWithQuorum` 加一个可选回调即可，UX 收益明显。
3. **claim/item 级别渐进式解析**——中，新增一个通用的增量数组提取状态机 + provider 流式接口，覆盖 propose/critique/revise/vote/normalize 五个结构化阶段，复用各阶段已有的子 schema 做校验，不改协议。这一步和第 4 步共享同一个 `completeStream` provider 接口，建议一起设计接口、分两次落地。
4. **compose 阶段 token 级流式**——中，只做用户直接阅读的最终答案，注意 token 事件要走"广播不落库"的旁路，避免 `run_events` 写放大。
5. **多模态输入**——中大，仅影响 propose 入口，五个后续 phase 的 schema 不用动。
6. **Tool calling（propose + critique）**——中大，两阶段共用同一套单轮工具机制；风险点是同质化和成本而不是协议冲突，建议做成运行时可选开关、默认关闭。
...
- Tool calling 的 `web_search` 结果要不要在同一个 run 内跨模型共享/缓存一份，以缓解"多个模型查同一件事、答案趋同"的问题？共享会降低成本和延迟，但也可能进一步削弱 propose 阶段本应有的独立性。
- propose 和 critique 的工具开关是做成一个开关（同开同关）还是两个独立开关？两个独立开关更灵活（比如只想让 critique 核实事实，不想让 propose 用工具），但会在发起 run 的表单里多一个选项，需要评估是否值得。
- 多模态输入的图片是否需要长期保留（影响存储选型）还是只在 run 生命周期内使用（内联 base64 即可，配合现有的匿名 workspace 30 天清理脚本 `db/cleanup.ts` 一起过期)？
